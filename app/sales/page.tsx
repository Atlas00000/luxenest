...
import { ChevronRight, Star, ShoppingBag, Percent } from "lucide-react"
...
export default function SalesPage() {
  ...
  const groupedProducts = saleProducts.reduce((acc, product) => {
  ...
  const discountGroups = Object.entries(groupedProducts).sort((a, b) => Number(b[0]) - Number(a[0]))
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">Special Offers</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Limited time deals on our most popular items
        </p>
      </motion.div>

      {discountGroups.map(([discount, products], groupIndex) => (
        <motion.div
          key={discount}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
